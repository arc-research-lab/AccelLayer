...
from .cdse_on_chip import *

def cdac_top(MODEL_IN,DATA_TYPE,num_acc,inter_assign,acc_trans_table,mem_num,HW_Part,term):
    ...
    if num_acc==1:
        ...
        final_config,time_layer,HW_Used,part_array,buff_use=cdse_on_chip(MODEL_IN,final_config,HW_Part,final_config_abc,acc_trans_table,acc,flag_last_acc,att_flag,Bdot_en,Bdot_num,DATA_TYPE,mem_num[acc],heads,term)
        return final_config, time_layer
    else:
        HW_LEFT=HW_Part[1:6].copy()
        HW_Used=np.zeros([5])
        HW_Cur=np.ones([6])*HW_Part[0]
        HW_temp=np.zeros([5])
        ...
        for i,acc in enumerate(index_mha):
            ...
            HW_temp[0:3]=np.multiply(HW_LEFT[0:3],comp_ratio)
            HW_temp[3:5]=np.multiply(HW_LEFT[3:5],mem_ratio)
            
            if HW_temp[1]<25:
                compensation[1] = 25 - HW_temp[1]
            if HW_temp[2]<20:
                compensation[2] = 20 - HW_temp[2]
            flag_last_acc = 0
            HW_Cur[1:6]=np.add(HW_temp,compensation)
            final_config[acc,:],time_layer,HW_Used,part_array,buff_use=cdse_on_chip(MODEL_PART,final_config,HW_Cur,final_config_abc,acc_trans_table,acc,flag_last_acc,att_flag,Bdot_en,Bdot_num,DATA_TYPE,mem_num[acc],heads,term)
            if final_config[acc,0]==max_int:
                print('No solution during dup initialization, acc:' + str(acc) ) 
                break
            dup = final_config[acc,31]
        ...
        for i,acc in enumerate(index):
            ...
            HW_temp[3:5]=np.multiply(HW_LEFT[3:5],mem_ratio)
            if i==num_acc-1:
                compensation=np.zeros([5])#AIE, PLIO_IN,PLIO_OUT,BRAM.URAM
                flag_last_acc=1
            elif Bdot_flag[acc]:
                if HW_temp[1]<25:
                    compensation[1] = 25-HW_temp[1]
                ...
                index_comp=np.where(HW_temp<=15)
                index_comp=list(index_comp[0])
                compensation[index_comp]=15
                flag_last_acc=0
            HW_Cur[1:6]=np.add(HW_temp,compensation)
            final_config[acc,:],time_layer,HW_Used,part_array,buff_use=cdse_on_chip(MODEL_PART,final_config,HW_Cur,final_config_abc,acc_trans_table,acc,flag_last_acc,att_flag,Bdot_en,dup_hint,DATA_TYPE,mem_num[acc],heads,term)
            # print("Acc" + str(acc), HW_Cur[1:6])
            # print("Acc %d, PLIO Used: %d, %d"%(acc,HW_Used[1],HW_Used[2]))
            # print("Acc %d, RAM Used: %d, %d"%(acc,HW_Used[3],HW_Used[4]))
            # view_config = np.zeros([16])
            # view_config[0:6] = final_config[acc,0:6].copy()
            # view_config[6:9] = final_config[acc,10:13].copy()
            # view_config[9:13] = final_config[acc,22:26].copy()
            # view_config[13:16] = final_config[acc,31:34].copy()
            # print('Hardware Configuration Acc ' + str(acc) + ' is\n h1, w1, w2, A, B, C, X, Y, Z, part_aL, part_bL, part_bR, part_cR, dup, atten_flag, harden_aie ')
            # print(view_config)
            # print(time_layer)
            # print('\n\n')
            if final_config[acc,0]==max_int:
            ...
            final_config_abc[acc,0:3]=final_config[acc,3:6].copy()
            final_config_abc[acc,3:7]=final_config[acc,22:26].copy()
            final_config_abc[acc,7]=final_config[acc,31].copy()
            layer_cycle[layer_index]=time_layer
            HW_LEFT = np.subtract(HW_LEFT,HW_Used)
            #print("Final config_abc is", final_config_abc)

            if i==num_acc-1:
                num_modified=part_array.shape[0]
                for i in range(num_modified):
                    acc=int(part_array[i,6])
                    final_config[acc,22:28]=part_array[i,0:6] # Update partition factors
                    final_config[acc,34:36]=buff_use[i,0:2]  # Update RAM Usage
